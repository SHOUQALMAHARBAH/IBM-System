...
|------|--------|-----------------|
| 2026-09-07 | **UPDATED `meta/context/part-e-dashboards.md`** — third of Part E's six named dashboards (backlog Process #64), after Sales and Policy. **Claims Dashboard built**: "open vs. closed claims, outstanding claims value, claims ageing, loss ratio by client/line/insurer" — a NEW `GET /dashboards/claims` endpoint, the FIRST real consumer of the pre-seeded `dashboard.claims.view` permission. **Every one of this bullet's four metrics is a CURRENT-STATE noun, not a period-scoped event** — unlike Sales/Policy, this dashboard carries no `periodStart`/`periodEnd` range at all; instead a single `asOf` reference-date override (the #33 AR ageing report's own shape) satisfies Part E's cross-cutting "filterable by time period" rule for a dashboard whose every metric is a live snapshot. `CLOSED` is the only terminal `ClaimStatus`, so "open" is simply "status != CLOSED." "Outstanding claims value" uses `Settlement.netSettlement` once known, else `Claim.estimatedLoss` — the same precedent `computeLossRatio` itself uses. "Claims ageing" buckets open claims by days since `createdAt` into new `d0_30`/`d31_60`/`d61_90`/`d90_plus` bands (no "not yet due" bucket, since an open claim ages from day zero). **"Loss ratio by client/line/insurer" required WIDENING Process 30's own `LOSS_RATIO_GROUP_BY`** (`['customer','policy','line']` → `+ 'insurer'`, with `AnalyticsPolicyLike`/`AnalyticsPolicyRow` gaining `insurerId`/`insurerName`) — a genuine, free capability upgrade to the pre-existing `GET /claims-analytics/loss-ratio?groupBy=` endpoint too, since its DTO validates against the same constant. The three breakdowns reuse the SAME pre-existing `buildLossRatioBreakdown` pure function via direct import — one step lighter than the #63 Profitability Analysis repository-injection precedent (no DI wiring for a pure function); `LossRatioRepository` itself IS reused the #63 way (independently provided in `ClaimsDashboardModule`'s own `providers`, no module-to-module wiring). Every metric filters uniformly by branch/line/insurer (every read goes through a `Claim`'s owning `Policy`) — no per-metric carve-out, like Policy Dashboard. **Verification**: +14 api unit (7 config, 6 service, +1 new insurer-groupBy test in `loss-ratio.config.spec.ts`) → api unit **2279** (181 files, from 2265). New `test/claims-dashboard.e2e-spec.ts` **5/5** — permission gating, a 400 on a malformed `asOf`, a 422 on a future `asOf`, a full open/closed/outstanding/ageing/loss-ratio walk isolated via a fresh `Insurer` row (this dashboard's `createdAt < asOf` scope has no lower bound, so a date window alone can't isolate — insurerId scoping does), and a branch-scoping test. Full api unit suite 2279/2279 confirmed green; full 60-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`, plus two confirmed-transient TOTP-timing flakes (`prospect`, `sales-performance`) re-confirmed clean in isolation. New Playwright `claims-dashboard.spec.ts` 3/3; full Playwright suite **273/273** (from 270). `npm run typecheck`/`lint`/`build` (api + web) OK. | Read `meta/context/part-e-dashboards.md` before touching any of Part E's remaining three dashboards — "a shared groupBy-style union type (like `LOSS_RATIO_GROUP_BY`) is meant to be widened when a new backlog item needs one more dimension — check whether the SAME union is already validated by an EARLIER endpoint's DTO before writing a dashboard-local copy, since widening it can upgrade that older endpoint for free", "when a dashboard's own bullet has ZERO period-scoped metrics (every noun is current-state), don't force a `periodStart`/`periodEnd` range onto it just because sibling dashboards have one — a single `asOf` reference-date override (the AR ageing report's own shape) is the honest interpretation of 'filterable by time period' for a pure live-snapshot dashboard", and "when a dashboard's own date filter has NO lower bound (an open-ended `createdAt < asOf` scope, unlike a period RANGE), a distinctive historical date window cannot isolate one e2e test's fixtures from `db-test`'s cumulative history on its own — scope by a freshly-created, uniquely-named dimension row (like a fresh `Insurer`) and filter by its id instead" are all easy to get wrong. |
