...
|------|--------|-----------------|
| 2026-09-07 | **UPDATED `meta/context/part-e-dashboards.md`** — fifth of Part E's six named dashboards (backlog Process #64), after Sales, Policy, Claims, and Financial. **Compliance Dashboard built**: "KYC status (approved/pending/overdue for refresh), complaints by status/category, compliance breaches/exceptions, regulatory filing/report status, open DSRs, breach-register status, DPIA backlog" — a NEW `GET /dashboards/compliance` endpoint, the FIRST real consumer of `dashboard.compliance.view`. **Seven sections, each reading a different existing table directly — the DPO Workspace "aggregate several registers, zero cross-module SERVICE dependency" shape**, reused here for Compliance's own audience (real overlap with DPO Workspace on DSR/DPIA/breach-register is expected, not a bug). **No period range at all — one step further than Claims/Financial's own `asOf` compromise**: `KYCRecord`/`Complaint` statuses move through 6-7 states over an entity's life, so a `createdAt` cutoff would misrepresent "status as of a date" far more than Claims Dashboard's own accepted limitation; only `Claim` has a status-history table, so genuine reconstruction is disproportionate here — deliberately omitted. **`branchId` is the ONE cross-cutting filter with real reach; `insuranceLine`/`insurerId` are omitted dashboard-wide** — none of the seven models ties to a `Policy` at all, the first Part E dashboard where a filter dimension fails to apply ANYWHERE, not just to some metrics. **"Compliance breaches/exceptions" needed a genuinely drafted interpretation** (the backlog names no model): `ReconciliationException` (#39) is FINANCE-owned, not Compliance's; `TransactionMonitoringAlert` (#48, AML/CFT) IS Compliance-owned (`aml.monitor`) and cheap to query — used. Internal Controls' own self-approval findings were a candidate too, but re-running its expensive 16-query live scan inline would be disproportionate — instead reads the MOST RECENT already-persisted `InternalControlsAuditReport` audit row via `AuditTrailRepository` (reused directly, no re-scan). KYC's "overdue for refresh" already exists as a real status (`PERIODIC_REVIEW_DUE`, driven live by the pre-existing `kyc-periodic-review.scheduler.ts`) — no derivation needed. "Open DSRs"/breach-register "open" reuse Claims Dashboard's own "CLOSED is the sole terminal status" rule; "regulatory filing status" reuses #51's own `deriveComplianceCalendarItemView` pure function directly. **Verification**: +16 api unit (10 config, 6 service) → api unit **2303** (185 files, from 2287). New `test/compliance-dashboard.e2e-spec.ts` **4/4** — permission gating, a branch-scoped walk (KYC/complaints/DSR/AML sections isolated exactly), a BEFORE/AFTER delta walk for the three sections with no owner dimension (regulatory filings/breach-register/DPIA backlog, the DPO Workspace precedent), and a direct-audit-row test proving the self-approval read is NOT a live re-scan. Full api unit suite 2303/2303 confirmed green; full 62-file api e2e suite green across 8 foreground sub-batches — the system ran roughly 2x slower than prior sessions throughout this verification pass, and the chronic `rbac` flake needed a 180s re-run (still well within its own logic, confirmed clean) to rule out a genuine regression rather than machine load. New Playwright `compliance-dashboard.spec.ts` 3/3; full Playwright suite **279/279** (from 276). `npm run typecheck`/`lint`/`build` (api + web) OK. No migration, no widening of any existing repository — unlike Financial Dashboard, every table this dashboard reads already supported everything it needed. | Read `meta/context/part-e-dashboards.md` before touching the last Part E item (verifying Insurer & Employee Performance) — "a cross-cutting filter dimension (insuranceLine/insurerId here) can fail to apply to an ENTIRE dashboard, not just to some of its metrics — check every underlying model's own relations before assuming a Part E dashboard needs the full branch/line/insurer/period filter set", "when a backlog bullet names a concept with no obvious model (here, 'compliance breaches/exceptions'), check which of several plausible candidate models is actually OWNED by the dashboard's own audience role (grep the permission seed file for who holds `X.manage`/`X.monitor`) rather than picking whichever model has the most fitting name", and "reading the MOST RECENT already-persisted audit row for an expensive live scan's last-known result, instead of re-running that scan inline, is a real and repeatable pattern whenever a dashboard wants to surface a periodic audit's own findings without paying its full cost on every page load" are all easy to get wrong. |
