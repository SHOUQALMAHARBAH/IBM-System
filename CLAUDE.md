...
|------|--------|-----------------|
| 2026-09-07 | **NEW `meta/context/part-e-dashboards.md`** — opens Part E (Dashboards & Management Reporting, Part 13), backlog Process #64 ("Executive Management Reporting — Part E below," i.e. Part E IS #64's full detail, not six unrelated processes). Worked one dashboard at a time (the Part D pacing default resumed after its one-time "finish everything" override). **Sales Dashboard built**: "new leads and conversion rate, premium written (new vs. renewal), commission income, cross-sell/up-sell opportunity conversion" — a NEW `GET /dashboards/sales` endpoint, reusing the ALREADY-CONSUMED `dashboard.sales.view` permission (backlog #59's own `GET /sales-performance` uses the same code for a narrower quota-tracking metric; the two endpoints deliberately coexist, not merged). **Filter applicability is real, not uniform** — the cross-cutting "filterable by branch/line/insurer/period" rule is honored only where each dimension exists on the underlying model (documented per-metric in the new file), not force-fit everywhere: cross-sell/up-sell have no branch/insurer dimension at all (system-detected against a Customer, no officer-ownership concept). **"Premium written (new vs. renewal)" splits on `Opportunity.isRenewal`** — a real boolean already on the schema, found by checking rather than assuming new modeling was needed — computed via two `Policy.aggregate()` relation-filtered calls (a genuine DB-side sum) combined with `addMoney()`/`formatMoney()`, never by parsing formatted money strings back into JS floating-point numbers. No migration, no new permission, no cross-module service dependency (the #58 KPI Dashboard / #62 Portfolio Analysis "read tables directly" shape). **Verification**: +16 api unit → api unit **2254** (177 files, from 2238). New `test/sales-dashboard.e2e-spec.ts` **4/4** — permission gating; a 422 on a partial period override; a full leads/premium(new-vs-renewal)/commission/cross-sell/up-sell walk against a distinctive historical period window; a branch-scoping test proving cross-sell/up-sell stay unscoped. Full api unit suite 2254/2254 confirmed green; full 58-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `sales-dashboard.spec.ts` 3/3; full Playwright suite **267/267** (from 264). `npm run typecheck`/`lint`/`build` (api + web) OK. | Read `meta/context/part-e-dashboards.md` before touching any of Part E's remaining five dashboards — "Part E's own header cross-references backlog Process #64 directly ('Part E below') — check whether a Part-lettered section is actually restating a numbered Domain process before assuming it needs its own independent scoping", "a pre-seeded `dashboard.*.view` permission already consumed by an EARLIER backlog item (here, #59) for a NARROWER metric set doesn't mean the permission is used up — a later item can deliberately reuse the same code for a genuinely separate, broader endpoint rather than extending the existing one", and "'filterable by X/Y/Z' in a cross-cutting requirement does not mean literally every filter must apply to literally every metric — check whether each dimension actually exists on the underlying model before force-fitting it, and document the real 'not applicable' cases rather than silently dropping the filter or silently applying it where it doesn't belong" are all easy to get wrong. |
