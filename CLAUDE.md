...
|------|--------|-----------------|
| 2026-09-07 | **UPDATED `meta/context/part-e-dashboards.md`** — second of Part E's six named dashboards (backlog Process #64), after Sales. **Policy Dashboard built**: "active policies, expiring policies (renewal window), new policies issued, cancelled policies and cancellation reasons" — a NEW `GET /dashboards/policy` endpoint, the FIRST real consumer of the pre-seeded `dashboard.policy.view` permission (unlike Sales, no earlier backlog item had touched this code). **Two of the four metrics are a LIVE snapshot as of `now` (active, expiring), two are period-scoped (new-issued, cancelled)** — a deliberate asymmetry read directly off the backlog bullet's own mixed phrasing (a snapshot phrase and two historical-event phrases in one sentence), not an inconsistency to fix. **"Cancelled policies" scopes by `Endorsement.appliedAt`, not `Cancellation.createdAt`** — confirmed by reading `EndorsementService.apply()`'s cancellation branch directly: the `Policy.status -> CANCELLED` transition fires exactly when `appliedAt` is stamped, while `Cancellation.createdAt` reflects only when the cancellation was requested (which can predate the period it actually took effect in). `reason` (free text) is a capped, ordered `findMany` list, never a `groupBy` — the #62 Portfolio Analysis "don't group free text" precedent. The renewal window defaults to 90 days, reusing `RenewalCase.leadTimeDays`'s own existing default rather than inventing a new figure, exposed as a caller-overridable `renewalWindowDays` query param. Unlike Sales, every one of the four metrics reads the same `Policy` table, so branch/line/insurer filters apply uniformly across all four — no per-metric carve-out needed this time. No migration, no new permission, no cross-module service dependency. **Verification**: +11 api unit → api unit **2265** (179 files, from 2254). New `test/policy-dashboard.e2e-spec.ts` **4/4** — permission gating; a 422 on a partial period override; a full active/expiring/newly-issued/cancelled walk against a distinctive historical period window with a real `Endorsement`+`Cancellation` fixture; a `renewalWindowDays` override test. Full api unit suite 2265/2265 confirmed green; full 59-file api e2e suite green across 8 foreground sub-batches, both chronic flakes (`rbac`, `up-sell`) passing with `--testTimeout=90000`. New Playwright `policy-dashboard.spec.ts` 3/3; full Playwright suite **270/270** (from 267) — one confirmed-transient Chromium crash on an unrelated `retention-disposal.spec.ts` a11y test, re-confirmed clean in isolation. `npm run typecheck`/`lint`/`build` (api + web) OK. | Read `meta/context/part-e-dashboards.md` before touching any of Part E's remaining four dashboards — "a dashboard's own metrics can legitimately mix LIVE-SNAPSHOT and PERIOD-SCOPED temporality on the same screen — read the backlog bullet's own phrasing (a snapshot verb vs. a historical-event verb) to decide which each metric is, rather than force-fitting every metric onto one shared period filter", "when a workflow-transition status change (like `Policy.status -> CANCELLED`) and a related child record's own `createdAt` (like `Cancellation.createdAt`) could both plausibly serve as 'the date this happened,' read the actual service code that performs the transition to find which field is stamped at the moment the status change fires — don't assume the child record's creation timestamp is authoritative", and "a cross-cutting 'filterable by branch/line/insurer' rule can legitimately apply UNIFORMLY across every metric on one dashboard while needing per-metric carve-outs on a sibling dashboard in the same Part E — check what tables the metrics actually read rather than copying the previous dashboard's documented exceptions" are all easy to get wrong. |
