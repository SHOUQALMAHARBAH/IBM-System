...
- **An aggregate / monitoring read (a dashboard, an analytics endpoint, a cross-module report) that does not set `isSensitiveDataAccess` on its `READ` audit row when the result actually touched a personal-data-bearing record.** Part 10.3 logs *read access to* Sensitive Personal Data — and for an aggregate the accountable fact is **existence-context**: that a DSR, complaint, claim, consent-withdrawal, KYC file or breach record *exists for a data subject* is itself sensitive, even when the response exposes only counts and no field content. So the flag is flipped by the presence of such a record in the loaded/aggregated set, not by whether a masked field was rendered. Precedents: `ClaimsAnalyticsService` (#30, `entityType: 'ClaimsAnalytics'`), `FinancialReportService` (#40, `'FinancialReport'`), `SlaDashboardService` (#43, `'SlaDashboard'` + `SLA_DASHBOARD_SENSITIVE_ENTITY_TYPES`). The audit `afterValue` still carries counts / filter *type names* only — never a record id or a person's name. **The canonical list of "personal-data-bearing entity types" is not yet defined once** — #30 / #40 / #43 each carry their own; consolidate into the data dictionary (`PRIV-SRS-02`) when its field/entity names land, and until then keep the per-module list explicit and reviewed, not inferred.
